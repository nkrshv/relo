...
MOVE-LOGISTICS MODULES — mandatory in EVERY plan, each in the phase named. These are the universal cross-border logistics people forget; make each one destination-specific. NEVER silently drop a module: when a phase's item cap is tight, MERGE related modules into one item that covers both (e.g. the 2FA/recovery-codes work inside the digital & financial continuity item, the home health-insurance wind-down inside the deregistration item, the tax calendar inside the tax notification item) — merged coverage counts, dropped coverage does not:
...
- "before" — DOCUMENT LEGALIZATION: identify which civil and education documents (birth certificate, marriage certificate, diplomas/transcripts, criminal-record check) this destination requires with an Apostille or, for non-Hague countries, consular legalization, and often a sworn translation. State plainly that an Apostille can ONLY be issued by the country that ISSUED the document, so it must be done at home before leaving; getting it from abroad means couriers, a power of attorney, or a trip back. When the destination's visa or residence route requires a CRIMINAL-RECORD CHECK, obtaining it from the origin country is mandatory in this item, using the origin country's real issuing body and process (from the verified origin-country facts when provided): it is one of the slowest documents to get from abroad and usually also needs the apostille/legalization chain on top.
...
4. If "deadline" is empty but a legal deadline exists — add it. In housing/accommodation items, DELETE any mention of a specific listing portal, marketplace, or agency name from every field (title, why, tip, steps) and set their url to "": describe only channel categories (established local listing portals, licensed agents) and vetting rules. commonMistake audit: any commonMistake that would read true for almost any task ("don't forget", "delaying causes complications", "not doing it on time", "failing to register results in fines", "waiting too long", "not being prepared") is filler and MUST be replaced with the real, step-specific trap and its concrete consequence: the exact wrong action or reversed order of operations, and which later step it blocks / how many weeks or how much money it costs / the penalty or disqualifying threshold.
