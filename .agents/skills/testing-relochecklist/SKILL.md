...
- "What to pack" renders as a checkable section at the BOTTOM of the checklist (ids `pack:<i>`, shared `relochecklist:checked` localStorage map). Checking an item must bump the overall progress rail total (e.g. 0/19 → 1/19) and survive a reload. Note it's in Simple view only, not injected into `plan.phases` (Advanced table won't show packing rows — by design so far).
