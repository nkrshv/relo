...
- **No payment card data on our servers.** Payments are handled entirely by our payment processor (Stripe); we never see or store your card number.
...
The origin country, destination country, city, mover profile, timeline, priorities, budget note, and free-text notes you enter into the plan form. This data is:
...
- **Not stored in a user account** because there are no accounts. Your generated plan is saved **in your own browser's local storage** so you can return to it; it is under your control and you can clear it at any time via your browser settings.

### Generated Content (your plan)
The checklist returned to you is delivered to your browser and cached locally in your browser. We do not build a server-side profile of you from it. Note that the generated plan is one of the "outputs" that may be shared with OpenAI to improve and train its services (see §4).
...
### Analytics Data
We use Google Analytics 4 (loaded through Google Tag Manager), Vercel Analytics, and Vercel Speed Insights to understand aggregate usage (page views, approximate country-level location, device/browser type, referrer, session duration, and web-performance metrics). These tools measure trends; we do not use them to identify you personally.

### Payment Data
When you buy the full plan, payment is processed by **Stripe**. Stripe collects the information needed to process the transaction (for example your card details and billing information) under **Stripe's** own privacy policy. We receive only a confirmation that payment succeeded, not your card number.

## 4. How We Use Information

- **Provide the service:** generate and return your relocation checklist.
- **Process payments:** confirm your purchase via Stripe and unlock the full plan.
...
|--------|---------|----------|----------------|
| `_ga`, `_ga_*` | Google Analytics (aggregate usage measurement) | Up to 2 years | [GA opt-out add-on](https://tools.google.com/dlpage/gaoptout) or block third-party cookies |
...
- Because Reloka stores your generated plan in your browser's local storage (not a cookie), clearing your browser data removes it.
...
|----------|---------|----------------|
| Vercel | Hosting, content delivery, analytics, performance | https://vercel.com/legal/privacy-policy |
| OpenAI | AI generation of plan text (API) | https://openai.com/policies/privacy-policy |
| Stripe | Payment processing | https://stripe.com/privacy |
| Google (Analytics / Tag Manager) | Aggregate usage analytics | https://policies.google.com/privacy |

Reloka also draws on public, open reference datasets (for example passport/visa data, World Bank indicators, OECD, air-quality, and network-measurement sources) to enrich plans. These are consulted as data sources and do not receive your personal inputs.

## 7. Data Retention

- **Plan inputs:** transmitted to the AI provider to generate your plan and not retained by us in an identifiable user record. Your plan is stored in **your** browser until you clear it.
- **"Request a country" entries:** the requested country name is retained until we have acted on the request.
- **Analytics aggregates:** retained per the analytics provider's defaults (Google Analytics 4 default is up to 14 months).
- **Server access logs:** retained for a short period (approximately 30 days) for security and debugging.
- **Payment records:** retained by Stripe and, to the extent we hold transaction confirmations, for as long as required for tax and accounting purposes.
...
- **Categories collected:** identifiers limited to approximate location and device/analytics data; commercial information (purchase of the full plan); internet activity (usage analytics). We do not collect sensitive personal information.
